...
#DB
client = InfluxDBClient(host='192.168.60.108', port=8086, database='claymore-DB')
...
# miner
miner_ip = '192.168.50.34'
miner2_ip='192.168.50.32'
miner_port = 3333
...
def wait_for_connection():
	...
	while True:
		try:
			client.create_database('claymore-DB')
			break
...
def main():
    ...
    while True:
        ...
        try:
            ...
            powerConsumed = data['result'][17]
            totalHashrate = int(data['result'][2].split(';')[0])
            totalShares = int(data['result'][2].split(';')[1])
            invalid_shares = data['result'][11].split(';')
            shares =  data['result'][9].split(';')
        except Exception as e:
            retries += 1
            if retries < 3:
